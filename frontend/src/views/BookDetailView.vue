<template>
  <section>
    <div class="container" v-if="!isLoading">
      <SectionHeader :title="book.title" :text="book.author" />
      <font-awesome-icon
        icon="arrow-left"
        size="2xl"
        class="mb-2"
        style="cursor: pointer"
        @click="gotoBackBooks"
      />
      <div class="row mb-5">
        <div class="col-lg-6">
          <img
            class="card-img-top"
            src="../../template/images/b_detail.jpg"
            alt=""
          />
        </div>
        <div class="col-lg-6 details-wrapper">
          <p class="lead description">
            {{ book.description }}
          </p>
          <div class="mb-4">
            <div class="row border-bottom pb-2">
              <div class="col-lg-6"><strong>Page</strong></div>
              <div class="col-lg-6">{{ book.page }}</div>
            </div>
            <div class="row border-bottom pb-2">
              <div class="col-lg-6"><strong>Category</strong></div>
              <div class="col-lg-6">{{ book.category }}</div>
            </div>
            <div class="row border-bottom pb-2">
              <div class="col-lg-6"><strong>Rating</strong></div>
              <div class="col-lg-6">{{ book.rating }}</div>
            </div>
            <div class="row border-bottom pb-2">
              <div class="col-lg-6"><strong>Uploaded Date</strong></div>
              <div class="col-lg-6">{{ book.createdAt }}</div>
            </div>
          </div>

          <!-- Comments Area -->
          <div class="comments-section">
            <h3 class="display-6 mb-5">Comments</h3>
            <div class="card mb-2">
              <div class="card-body">
                <p>
                  Lorem ipsum dolor, sit amet consectetur adipisicing elit.
                  Rerum, mollitia.
                </p>
                <div class="d-flex justify-content-between">
                  <p class="fw-bold fst-italic">John Doe</p>
                  <div class="d-flex align-items-center">
                    <font-awesome-icon :icon="['far', 'thumbs-up']" />
                    <p class="ps-2 mb-0"><strong>5</strong></p>
                  </div>
                </div>
              </div>
            </div>
            <div class="card mb-2">
              <div class="card-body">
                <p>
                  Lorem ipsum dolor, sit amet consectetur adipisicing elit.
                  Rerum, mollitia.
                </p>
                <div class="d-flex justify-content-between">
                  <p class="fw-bold fst-italic">John Doe</p>
                  <div class="d-flex align-items-center">
                    <p>Upvote</p>
                    <p class="ps-2"><strong>5</strong></p>
                  </div>
                </div>
              </div>
            </div>
            <div class="card mb-2">
              <div class="card-body">
                <p>
                  Lorem ipsum dolor, sit amet consectetur adipisicing elit.
                  Rerum, mollitia.
                </p>
                <div class="d-flex justify-content-between">
                  <p class="fw-bold fst-italic">John Doe</p>
                  <div class="d-flex align-items-center">
                    <p>Upvote</p>
                    <p class="ps-2"><strong>5</strong></p>
                  </div>
                </div>
              </div>
            </div>

            <div class="card mb-2">
              <div class="card-body">
                <p>
                  Lorem ipsum dolor, sit amet consectetur adipisicing elit.
                  Rerum, mollitia.
                </p>
                <div class="d-flex justify-content-between">
                  <p class="fw-bold fst-italic">John Doe</p>
                  <div class="d-flex align-items-center">
                    <p>Upvote</p>
                    <p class="ps-2"><strong>5</strong></p>
                  </div>
                </div>
              </div>
            </div>
            <div class="card mb-2">
              <div class="card-body">
                <p>
                  Lorem ipsum dolor, sit amet consectetur adipisicing elit.
                  Rerum, mollitia.
                </p>
                <div class="d-flex justify-content-between">
                  <p class="fw-bold fst-italic">John Doe</p>
                  <div class="d-flex align-items-center">
                    <p>Upvote</p>
                    <p class="ps-2"><strong>5</strong></p>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div class="container" v-else>
      <p>Book Detail Loading...</p>
    </div>
  </section>
</template>

<script>
import SectionHeader from "@/components/SectionHeader.vue";
import { useBookStore } from "@/stores/bookStore";
import { mapState } from "pinia";

export default {
  name: "BookDetailView",
  components: {
    SectionHeader,
  },
  data() {
    return {
      book: null,
      isLoading: true,
    };
  },
  created() {
    this.selectBook();
  },
  methods: {
    gotoBackBooks() {
      this.$router.push("/books");
    },

    selectBook(book) {
      const bookId = this.$route.params.id;
      this.book = this.selectedBook(bookId);
      this.isLoading = false;
    },
  },
  computed: {
    ...mapState(useBookStore, ["selectedBook"]),
  },
};
</script>

<style scoped>
.description {
  min-height: 150px;
  max-height: 250px;
  overflow-y: auto;
}

.details-wrapper {
  max-height: 740px;
  display: flex;
  flex-direction: column;
}

.comments-section {
  flex-grow: 1;
  overflow-y: auto;
}
</style>
